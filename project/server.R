...
# Define server logic required to summarize and view the selected question
shinyServer (function (input, output, session){
        ...
        # returs input$caption.  When the user changes the "caption"field:
        #
        # 1) This expression is automatically called to recompute the output
        # 2) The enw caption is pushed back to the browser for re-display
        #
                        ...
                        if (input$selectinput == "Question2"){
                                ...
                                if (input$selectinput == "Question3"){
                                        ...
                                        isolate (dataset <<- ggplot (emissionsBaltimore,
                                                         ...
                                                         ggtitle ("PM2.5 emission - in the Baltimore City,
                                                                  Maryland - 1999 - 2008 - By Source Type"))
                                        print (dataset)
                                }
                        }
                }
        ...
        # The output$view depends of the datasetInput reactive expression
        output$view <- renderTable ({
                if (input$selectinput != "All Questions") {
                        output$observations <- renderText ({ paste ("Observations")})
                        head (NEI, n = input$obs)
                }
        })
})
